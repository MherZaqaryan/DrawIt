...
public class GameSelector extends UniqueMenu {
    ...
    public void handleMenu(InventoryClickEvent e) {
        ...
        if (!nbtItem.getString("name").equals("game-menu")) return;
    ...
    public void setMenuItems() {
        Arrays.stream(slots).forEach(slot -> inventory.setItem(slot, DrawIt.getConfigData().getItem(ConfigData.GAMES_MENU_SETTINGS_WAITING_SLOTS, null)));

        if (DrawIt.getConfigData().getGameMenuItems().isEmpty()) return;

        for (ItemStack item : DrawIt.getConfigData().getGameMenuItems()) {
            if (item == null || item.getType().equals(Material.AIR)) continue;
            NBTItem nbti = new NBTItem(item);
            if (!nbti.getString("name").equals("game-menu")) continue;
        ...
        for (Game game : new ArrayList<>(DrawIt.getInstance().getGames())) {
            ...
            if (game.isGameState(GameState.WAITING)) {
                ...
                if (!DrawIt.getConfigData().getBoolean(ConfigData.GAMES_MENU_SETTINGS_SHOW_PLAYINGS)) return;
